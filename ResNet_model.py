...
def compute_model_density(model: torch.nn.Module) -> float:
    """
    Compute the density of a PyTorch model defined as:
    (number of parameters) / (model size in bytes)

    Returns:
        float: density (parameters per byte)
    """
    # Total number of parameters
    num_params = sum(p.numel() for p in model.parameters())

    # Estimate size in bytes assuming all parameters are float32 (4 bytes)
